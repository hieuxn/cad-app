...
export class ObjectControlService {
  ...
  /** Call this after three.js scene init completely */
  init() {
    this._selectionService = this._injector.get(ObjectSelectionService);
    this._mainView3DService = this._injector.get(MainView3DService);
    ...
    this._subscription.add(sub);
    this._dragControls = new DragControls(this._injector);
    this._rotateControls = new RotateControls(this._injector);
  }
  ...
  private _onObjectHovered(event: { change: string, changedItem: Object3D }) {
    if (event.change === 'add') {
      this._mainView3DService.renderer.domElement.style.cursor = 'pointer';
    }
    else {
      this._mainView3DService.renderer.domElement.style.cursor = 'auto';
    }
  }
}
...
class RotateControls {
  ...
  constructor(private _injector: Injector) {
    ...
    this._objectCreatorService = _injector.get(ObjectCreationService);
  }
  ...
  private _onMouseMove(event: MouseEvent) {
    ...
    //     ((child as Line)?.geometry as BufferGeometry)?.applyMatrix4(obj.matrix);
    //   }
