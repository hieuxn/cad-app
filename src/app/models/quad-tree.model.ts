...
export interface Vector2Ref extends Vector2 {
  ref: Object;
}

export class QuadTreeRef<T extends Vector2Ref> extends QuadTree<T>{
  private _map = new Map<Object, T[]>();

  removeByRef(object: Object): boolean {
    const points = this._map.get(object);
    if (undefined === points) return false;
  ...
  insertRef(data: T) {
    let points = this._map.get(data.ref);
    if (undefined === points) {
      points = [];
      this._map.set(data.ref, points)
    }
    points.push(data);
    this.insert(data);
  }
